...
/** Deterministic reproduction attempt for issue #10.
  ...
  * in-memory store never models: latency between capturing now and reading,
  * and between deciding and writing. Those gaps are where a stale now can
  * commit behind a newer writer, which is the only interleaving I could not
  * rule out on paper.
  */
class TokenBucketOccBoundSpec
  ...
  private final class OccItem(
    ...
    def grantWindow: IO[(Option[Long], Long)] = (createdAt.get, lastGrantNow.get).tupled
  ...
  "token bucket under OCC" - {
    ...
    "never admits more than capacity + rate * elapsed, under adversarial latency" in {
      ...
      TestControl.executeEmbed(test).asserting { case (c, elapsedMs, created, lastGrant) =>
        val elapsedSec = elapsedMs / 1000.0
        // The bound loadSim asserts: the client's window, zero epsilon because
        // virtual time is exact.
        val clientCeiling = profile.capacity + profile.refillRatePerSecond * elapsedSec
        ...
        val serverCeiling =
          profile.capacity + profile.refillRatePerSecond * (lastGrant - createdMs) / 1000.0

        withClue(
          s"counts=$c elapsed=${elapsedSec}s client=$clientCeiling server=$serverCeiling: ",
        ) {
